...
// ---------------------------------------------------------------------------
// Heavy-ball IRIS kernel
//   var     <- EMA of g_t^2     (denom for g_est term)
//   innov_var <- EMA of I_t^2   (denom for innov_res term)
//   update: x -= lr * (g_est/sqrt(v) + beta2 * innov_res/sqrt(n))
// ---------------------------------------------------------------------------
template <typename scalar_t>
__global__ void iris_heavyball_kernel(
    scalar_t* __restrict__ params,
    const scalar_t* __restrict__ grads,
    scalar_t* __restrict__ grad_estimates,
    scalar_t* __restrict__ variance_estimates,
    scalar_t* __restrict__ innovation_residuals,
    scalar_t* __restrict__ innovation_variances,
    const int64_t numel,
    const scalar_t lr,
    const scalar_t psi_inv_1,
    const scalar_t psi_inv_2,
    const scalar_t psi_inv_3,
    const scalar_t phi_inv,
    const scalar_t beta2,
    const scalar_t wd,
    const scalar_t eps
) {
    ...
    for (int64_t i = tid; i < numel; i += blockDim.x * gridDim.x) {
        ...
        scalar_t var_est   = variance_estimates[i];
        scalar_t innov_res = innovation_residuals[i];
        scalar_t innov_var = innovation_variances[i];
        const scalar_t scalar_t1 = scalar_t(1);
        ...
        // innov_res = lerp(prev, innovation, psi_inv_2)
        innov_res = (scalar_t1 - psi_inv_2) * innov_res + psi_inv_2 * innovation;
        innovation_residuals[i] = innov_res;

        // var: EMA of g_t^2
        var_est = (scalar_t1 - psi_inv_3) * var_est + psi_inv_3 * grad * grad;
        variance_estimates[i] = var_est;

        // innov_var: EMA of I_t^2
        innov_var = (scalar_t1 - phi_inv) * innov_var + phi_inv * innovation * innovation;
        innovation_variances[i] = innov_var;

        const scalar_t denom_v = sqrt(static_cast<float>(var_est))   + eps;
        const scalar_t denom_n = sqrt(static_cast<float>(innov_var)) + eps;
        ...
        // split update: g_est / sqrt(v) and beta2 * innov_res / sqrt(n)
        param -= lr * (grad_est / denom_v);
        param -= lr * beta2 * (innov_res / denom_n);
...
// ---------------------------------------------------------------------------
std::tuple<double, double, double, double> iris_fused_cuda(
    at::Tensor params,
    at::Tensor grads,
    ...
    at::Tensor innovation_variances,   // may be undefined when not in split_correction mode
    ...
    double beta2,
    double beta3,
    double gamma,     // split_correction coefficient (0.0 when not used)
    double wd,
    double eps,
    double rho,
    bool use_clipping,
    ...
    double phi_curr   = use_heavyball ? gamma * phi_prev + 1.0 : 0.0;

    double psi_inv_1 = 1.0 / psi_1_curr;
    double psi_inv_2 = 1.0 / psi_2_curr;
    double psi_inv_3 = 1.0 / psi_3_curr;
    double phi_inv   = use_heavyball ? 1.0 / phi_curr : 0.0;

    auto stream = at::cuda::getCurrentCUDAStream();

    if (use_heavyball) {
        TORCH_CHECK(innovation_variances.defined(), "innovation_variances required for split_correction mode");
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(params.scalar_type(), "iris_heavyball_kernel", [&] {
            iris_heavyball_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
    ...
    return std::make_tuple(psi_1_curr, psi_2_curr, psi_3_curr, phi_curr);
}

// ---------------------------------------------------------------------------
// Multi-tensor entry point (called from Python fused API)
// ---------------------------------------------------------------------------
std::tuple<double, double, double, double> iris_multi_tensor_fused_cuda(
    ...
    std::vector<at::Tensor> innovation_variances,   // empty when not in split_correction mode
    ...
    for (size_t i = 0; i < n_tensors; i++) {
        ...
        if (use_heavyball) {
            AT_DISPATCH_FLOATING_TYPES_AND_HALF(params[i].scalar_type(), "iris_heavyball_kernel", [&] {
                iris_heavyball_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
                    ...
                    innovation_variances[i].data_ptr<scalar_t>(),
                    numel,
                    static_cast<scalar_t>(lr),
                    static_cast<scalar_t>(psi_inv_1),
                    static_cast<scalar_t>(psi_inv_2),
